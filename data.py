...
class CocoDataset(data.Dataset):
    ...
    def __init__(self, root, json, vocab, transform=None, ids=None, drop_prob=0):
        """
        Args:
...
class CocoDatasetFast(data.Dataset):
...
class CocoDatasetFast_1x1(data.Dataset):
    ...
    def __len__(self):
        return len(self.img_ids) * 5


    def __getitem__(self, index):
        vocab = self.vocab
        ann_id, ann, path, image = self.get_raw_item(index)
        if self.transform is not None:
            image = self.transform(image)
        ...
        def tokenize(sentence):
            ...
            if sys.version_info.major > 2:
                ...
                tokens = nltk.tokenize.word_tokenize(str(sentence).lower().decode('utf-8'))
            return torch.Tensor([vocab('<start>')] + [vocab(token) for token in tokens] + [vocab('<end>')])
        
        ann = tokenize(ann)
        
        return image, ann, index, ann_id


    def get_raw_item(self, index):
        nth_img = int(index % len(self.img_ids))
        nth_txt = int(index / len(self.img_ids))
        is_beyond_bp = int(nth_img >= self.img_bp)
        coco, root, img_id_to_ann_ids = \
            self.coco[is_beyond_bp], self.root[is_beyond_bp], self.img_id_to_ann_ids[is_beyond_bp]
        
        img_id = self.img_ids[nth_img]
        ann_id = img_id_to_ann_ids[img_id][nth_txt]
        ann = coco.anns[ann_id]['caption']
        path = coco.loadImgs(img_id)[0]['file_name']
        image = Image.open(os.path.join(root, path)).convert('RGB')
        
        return ann_id, ann, path, image
...
class FlickrDatasetFast(data.Dataset):
    """
    Dataset loader for Flickr30k datasets.
    """
...
class PrecompRegionDataset(data.Dataset):
    ...
    def __init__(self, data_path, data_name, split, vocab, i_drop_prob, c_drop_prob):
        ...
        print('take:', time.time() - start,'  image_length:',len(self.images))
...
class PrecompRegionDatasetFast(data.Dataset):
    """
    Load precomputed captions and image features for COCO or Flickr
    """
...
class PrecompRegionDatasetFastBERT(data.Dataset):
    """
    Load precomputed captions and image features for COCO or Flickr
    """
...
def get_loader_single(data_name, split, root, json, vocab, transform,
    ...
    fast_batch = opt.fast_batch and split == 'train' # Use fast dataloader when only train mode
    fast_1x1 = opt.fast_1x1 and split == 'train'
    ...
    if 'coco' == data_name:
        ...
        if fast_batch:
            dataset = (PrecompRegionDatasetFast if not use_bert else PrecompRegionDatasetFastBERT)(
                data_path=root,
                data_name='coco_butd',
